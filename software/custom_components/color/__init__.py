...
from esphome.const import CONF_BLUE, CONF_GREEN, CONF_ID, CONF_RED, CONF_WHITE, CONF_COLOR
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_COLOR): cv.All(
            [cv.uint8_t], cv.Length(min=3, max=4)
        ),
    }
...
async def to_code(config):
    ...
    if CONF_COLOR in config:
        r = config[CONF_COLOR][0]
        g = config[CONF_COLOR][1]
        b = config[CONF_COLOR][2]
        if len(config[CONF_COLOR]) == 4:
            w = config[CONF_COLOR][3]
